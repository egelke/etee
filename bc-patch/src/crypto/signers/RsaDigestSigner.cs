...
namespace Org.BouncyCastle.Crypto.Signers
{
		...
		: ISigner
    {
        ...
        private RSACryptoServiceProvider proxy;
			...
			ICipherParameters	parameters)
        {
            ...
            if (k is ProxyRsaKeyParameters) {
                //Can do both, use proxy for signing and BC engine for verifying
                proxy = ((ProxyRsaKeyParameters)k).Proxy; //use for signing
                rsaEngine.Init(false, parameters); //use for verifying
            } else {
                //Can only do signing or validating
        ...
        public byte[] GenerateSignature()
        {
            ...
            if (proxy != null)
            {
                return proxy.SignHash(hash, algId.ObjectID.ToString());
            }
            else
            {
